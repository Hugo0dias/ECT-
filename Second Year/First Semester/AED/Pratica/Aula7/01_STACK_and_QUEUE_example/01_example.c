//
// TO DO : desenvolva um algoritmo para verificar se um numero inteiro positivo
//         e uma capicua
//         Exemplos: 12321 e uma capiacua, mas 123456 nao e
//         USE uma PILHA DE INTEIROS (pilha) e uma FILA DE INTEIROS (QUEUE)
//
// TO DO : design an algorithm to check if the digits of a positive decimal
//         integer number constitue a palindrome
//         Examples: 12321 is a palindrome, but 123456 is not
//         USE a STACK of integers and a QUEUE of integers
//
// v2
#include <stdio.h>

#include "IntegersQueue.h"
#include "IntegersStack.h"

int main(void) { 

    int num = 122222;
    Queue* fila = QueueCreate(50);
    Stack* pilha = StackCreate(50);

    while (num != 0)
    {
        int digito = num%10;
        num /= 10;
        QueueEnqueue(fila, digito);
        StackPush(pilha, digito);

    }
    
    while (!StackIsEmpty(pilha) && !QueueIsEmpty(fila))
    {
        if(StackPop(pilha) != QueueDequeue(fila)){
            printf("Nao e capicua\n");
            break;
        }
    }
    
    if(StackIsEmpty(pilha)){
        printf("E capicua\n");
    }

    StackDestroy(&pilha);
    QueueDestroy(&fila);

    return 0; 
    
}
