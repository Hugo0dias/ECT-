...
// ================================================================================== //
    ...
    void swpInit(SwappingPolicy policy)
    {
        ...
        SwpNode *InitialswpList = (SwpNode*)malloc(sizeof(SwpNode));
        if (!InitialswpList)
        {
            throw Exception(ENOSYS, "Failed to allocate memory for InitialswpList in swp_init.cpp");
        }

        InitialswpList->process.pid = 0;
        InitialswpList->process.size =  0;

        InitialswpList->next = nullptr;
